...
const Payroll = ({ onOpenCENTRALIZAT, auth, onLogout }) => {
    ...
    const fetchActiveEmployees = async () => {
        ...
        try {
            ...
            // Mocking Payroll Data on top of active employees
            const active = res.data
                .filter(app => app.status === 'Contratado')
                .map(app => ({
                    ...app,
                    payrollData: {
                        baseSalary: parseInt(app.workerData?.financial?.liquidSalary || 0),
                        bonuses: Math.floor(Math.random() * 100000), // Mock data
                        discounts: Math.floor(Math.random() * 50000), // Mock data
                        status: Math.random() > 0.3 ? 'Pendiente' : 'Pagado',
                    }
                }));

            // Calculate final liquid
            const enriched = active.map(app => ({
                ...app,
                payrollData: {
                    ...app.payrollData,
                    totalLiquid: app.payrollData.baseSalary + app.payrollData.bonuses - app.payrollData.discounts
                }
            }));
    ...
    const totalPayroll = employees.reduce((sum, emp) => sum + (emp.payrollData?.totalLiquid || 0), 0);
                ...
                <div className="bg-gradient-to-br from-indigo-600 to-indigo-800 p-8 rounded-[2.5rem] text-white shadow-xl shadow-indigo-200 col-span-1 md:col-span-2 relative overflow-hidden">
                    ...
                    <div className="relative z-10 flex flex-col h-full justify-between">
                        ...
                        <div>
                            ...
                            <p className="text-xs font-bold text-indigo-200 uppercase tracking-widest">Periodo: {new Date(currentPeriod + '-01').toLocaleDateString('es-ES', { month: 'long', year: 'numeric' })}</p>
                        </div>
                    </div>
                </div>
            ...
            <table className="w-full text-left border-collapse">
                <thead>
                    <tr className="bg-slate-50/50">
                        ...
                        <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 text-right">Sueldo Base</th>
                        <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 text-right">Haberes (+1)</th>
                        <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 text-right">Descuentos (-1)</th>
                        <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-800 bg-slate-100 text-right">Líquido a Pagar</th>
                                    ...
                                    <tr key={emp._id} className="group hover:bg-indigo-50/30 transition-all">
                                        <td className="px-8 py-6">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <p className="font-black text-slate-900 text-sm tracking-tight uppercase">{emp.fullName}</p>
                                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{emp.rut}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-8 py-6 text-right">
                                            <span className="font-bold text-slate-500">${emp.payrollData?.baseSalary.toLocaleString()}</span>
                                        </td>
                                        <td className="px-8 py-6 text-right">
                                            <span className="font-black text-emerald-600 bg-emerald-50 px-3 py-1.5 rounded-lg">+ ${emp.payrollData?.bonuses.toLocaleString()}</span>
                                        </td>
                                        <td className="px-8 py-6 text-right">
                                            <span className="font-black text-rose-600 bg-rose-50 px-3 py-1.5 rounded-lg">- ${emp.payrollData?.discounts.toLocaleString()}</span>
                                        </td>
                                        <td className="px-8 py-6 text-right bg-slate-50/50 group-hover:bg-indigo-50/50 transition-colors">
                                            <span className="text-lg font-black text-slate-900">${emp.payrollData?.totalLiquid.toLocaleString()}</span>
                                        </td>
                                        <td className="px-8 py-6 text-center">
                                            <span className={`px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest \${emp.payrollData?.status === 'Pagado' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
                    ...
                    <div className="bg-white rounded-[3rem] w-full max-w-lg shadow-2xl animate-in zoom-in duration-300 overflow-hidden">
                        ...
                        <div className="p-8 space-y-6">
                            ...
                            <div className="flex gap-4 pt-4">
                                ...
                                <button onClick={() => { toast.success('Ajuste aplicado correctamente'); setIsAdjustmentModalOpen(false); }} className={`px-8 py-4 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl transition-all flex-1 \${adjustmentType === 'Bonus' ? 'bg-emerald-600 hover:bg-emerald-700 shadow-emerald-200' : 'bg-rose-600 hover:bg-rose-700 shadow-rose-200'}`}>
