...
const HumanCapitalMaster = ({ auth, onLogout, onOpenCENTRALIZAT }) => {
    ...
    const handleAddNote = async () => {
        ...
        try {
            const { data } = await api.post(`/applicants/${selectedEmployee._id}/notes`, { note: newNote });
            // Update the selected employee's history in local state
            setSelectedEmployee({ ...selectedEmployee, history: data.history });
            // Also update the list state if needed
            setEmployees(employees.map(emp => emp._id === selectedEmployee._id ? { ...emp, history: data.history } : emp));
            setNewNote('');
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-2 space-y-8">
                        ...
                        {detailTab === 'history' && (
                            <div className="bg-white rounded-[2.5rem] p-10 shadow-sm border border-slate-100 animate-in fade-in duration-500">
                                <div className="flex justify-between items-center mb-8">
                                    <h3 className="text-xl font-black text-slate-800 uppercase tracking-tight">Línea de Vida del Colaborador</h3>
                                    <div className="px-4 py-2 bg-slate-50 text-slate-500 rounded-xl text-[10px] font-black uppercase tracking-widest border border-slate-100 italic">
                                        Audit Log en tiempo real
                                    </div>
                                </div>
                                <HistoryTimeline history={employee.history} />
                            </div>
                        )}
                    </div>
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="bg-slate-50 p-8 rounded-[2.5rem] border border-slate-100">
                            <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4 px-2">Notas del Ecosistema</h4>

                            {/* Collaborator Note Input */}
                            <div className="mb-6 space-y-3">
                                <textarea
                                    value={newNote}
                                    onChange={(e) => setNewNote(e.target.value)}
                                    placeholder="Añadir nota o feedback interno..."
                                    className="w-full p-4 bg-white border border-slate-200 rounded-2xl text-xs font-bold text-slate-700 focus:ring-4 focus:ring-indigo-500/10 outline-none resize-none h-24 placeholder:text-slate-300"
                                />
                                <button
                                    onClick={handleAddNote}
                                    disabled={isAddingNote || !newNote.trim()}
                                    className="w-full py-3 bg-indigo-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-indigo-700 transition-all disabled:opacity-30 flex items-center justify-center gap-2 shadow-lg shadow-indigo-200"
                                >
                                    {isAddingNote ? 'Guardando...' : <><Plus size={14} /> Registrar Nota</>}
                                </button>
                            </div>

                            <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4 px-2 italic">Flujo de Trabajo</h4>
